...
def knn(list_of_distances, list_of_species, k):
    ...
    num_for_species = []
    for i in range(check_species_number(list_of_species)):
        #wypełnienie listy ile jest możliwych gatunków
        num_for_species.append(0)
    for i in range(k):
    #wypełnienie tablicy tak, żeby było k elementów, działamy na indeksach, które mają odniesienie do listy odległości
        indexes_that_are_closest.append(i)
    for i in range(k - 1):
        for j in range(k - i - 1):
            if list_of_distances[indexes_that_are_closest[j]] > list_of_distances[
                #sortowanie tablicy od najmniejszej wartości odległości
    ...
    for i in range(k, len(list_of_distances)):
        for j in range(k):
            if list_of_distances[i] < list_of_distances[indexes_that_are_closest[j]]:
                ...
                indexes_that_are_closest[j] = i
                #sprawdzenie, czy badany element jest mniejszy od któregoś z elementów tablicy indeksów, jeśli tak to przesuwamy wszystkie elementy większe o jeden w prawo (oprócz ostatniego, który znika z tablicy) i wkładamy w odpiowiednie miejsce nr indeksowy tego elementu
                break
    for i in range(k):
